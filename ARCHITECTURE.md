...
**Dispatch all 4 leads tonight in parallel** — full build timeline + per-lead Day-1 deliverables + risk view in `docs/TIMELINE.md`. The "Bram waits for Aria's program" sequencing in an earlier draft was overly conservative: Bram can fully scaffold `services/automation/` with Trigger.dev config, strike-calculation unit tests, and mockable Pyth/Helius clients before Aria's program exists. Same for Cleo (Next.js + wallet-adapter shell) and Drew (test infra + simulation skeleton against a mocked Aria interface). The first integration gate is **Sat 5/23 6pm ET — Aria's first devnet deploy** — at which point all 4 leads converge for live integration.

Concrete Day-1 deliverables per lead are in `docs/TIMELINE.md`. Each lead's first task lives in their kickoff at `.project/bell-markets/kickoff/<name>.md` (local-only / OneDrive-mirrored, not in this repo). Dispatch entry point: open Cursor in each lead's worktree (`../BellMarkets-aria`, `-bram`, `-cleo`, `-drew`) and invoke `/aria`, `/bram`, `/cleo`, `/drew` respectively.
